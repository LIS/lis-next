...
install    -m0755 source/hv_kvp_daemon.service $RPM_BUILD_ROOT/lib/systemd/system/hv_kvp_daemon.service
install    -m0755 source/hv_kvp_daemon.service $RPM_BUILD_ROOT/lib/systemd/system/hv_fcopy_daemon.service
install    -m0755 source/hv_vss_daemon.service $RPM_BUILD_ROOT/lib/systemd/system/hv_vss_daemon.service
install -d -m0755 $RPM_BUILD_ROOT/usr/lib/udev/rules.d/
install    -m0755 source/70-hv_kvp.rules $RPM_BUILD_ROOT/usr/lib/udev/rules.d/70-hv_kvp.rules
install    -m0755 source/70-hv_fcopy.rules $RPM_BUILD_ROOT/usr/lib/udev/rules.d/70-hv_fcopy.rules
install    -m0755 source/70-hv_vss.rules $RPM_BUILD_ROOT/usr/lib/udev/rules.d/70-hv_vss.rules
...
if [ $1 -eq 2 ]; then
 ...
 # since we implemented new systemd for rhel7 we need to del old service for upgrade scenario.
 if [ -e /etc/init.d/hv_kvp_daemon ] 
    echo "Removing old Package.."
    /sbin/service hv_kvp_daemon stop > /dev/null 2>&1
    /sbin/chkconfig --del hv_kvp_daemon
    rm -rf /etc/init.d/hv_kvp_daemon
 fi
 if [ -e /etc/init.d/hv_vss_daemon ]
    /sbin/service hv_vss_daemon stop > /dev/null 2>&1
    /sbin/chkconfig --del hv_vss_daemon
    rm -rf /etc/init.d/hv_vss_daemon
 fi 
 if [ -e /etc/init.d/hv_fcopy_daemon ]
    /sbin/service hv_fcopy_daemon stop > /dev/null 2>&1
    /sbin/chkconfig --del hv_fcopy_daemon
    rm -rf /etc/init.d/hv_fcopy_daemon
 fi
fi
%post
...
if [ $1 -eq 0 ]; then # package is being erased, not upgraded
    ...
    rm /lib/systemd/system/hv_kvp_daemon.service
    rm  /usr/lib/udev/rules.d/70-hv_kvp.rules
    echo "Stopping FCOPY Daemon...."
    systemctl stop hv_fcopy_daemon
    rm /lib/systemd/system/hv_fcopy_daemon.service
    rm  /usr/lib/udev/rules.d/70-hv_fcopy.rules
    echo "Stopping VSS Daemon...."
    systemctl stop hv_vss_daemon
    rm /lib/systemd/system/hv_vss_daemon.service
    rm  /usr/lib/udev/rules.d/70-hv_vss.rules
